...
from federatedml.nn.backend.pytorch.data import TableDataSet, VisionDataSet, NLPDataSet
...
from federatedml.param import HomoNLPParam
...
class _PyTorchSAContext(object):
    ...
    def aggregation_iteration(self):
        LOGGER.info(f"_aggregation_iteration:{self._aggregation_iteration}")
...
class PyTorchSAClientContext(_PyTorchSAContext):
    ...
    def send_model(self, tensors, weight):
        LOGGER.info("client is sending model...")
    ...
    def recv_model(self):
        LOGGER.info(f"recv model ing ...{self._suffix()}")
        return [
    ...
    def send_loss(self, loss, weight):
        LOGGER.info("client is sending loss...")
    ...
    def do_aggregation(self, weight):
        ...
        LOGGER.info("client is doing aggregation...")
        agg_tensors: typing.List[torch.Tensor] = self.recv_model()
        LOGGER.info("recv_model is ok")
...
class FedLightModule(pl.LightningModule):
    def __init__(
        ...
        self.model = Bert.from_pretrained(
            'bert-base-chinese',
            config=config
        )
        ...
        self.train_to_device_cols = ['input_ids', 'attention_mask', 'token_type_ids', 'label_ids']
    ...
    def training_step(self, batch, batch_idx):
        ...
        for col in x:
            if type(x[col]) is torch.Tensor:
                x[col] = x[col].to(self.device)
            else:
                warnings.warn(f"The {col} is not Tensor.\n")
        # inputs = self._get_module_inputs_on_train(batch)
    ...
    def on_train_batch_start(self, batch: typing.Any, *args, **kwargs) -> None:
        # LOGGER.info("on_train_batch_start...")
        if self._all_consumed_data_aggregated:
            self._all_consumed_data_aggregated = False
    ...
    def configure_optimizers(self):
        ...
        LOGGER.info("optimizer is configured successfully!")
        return optimizer
...
def make_dataset(data, **kwargs):
    ...
    # train_data_path = '/data/projects/fate/examples/dsl/v2/homo_nn/KUAKE-QIC_train.json'
    dataset = NLPDataSet(data.path,**kwargs)
    return dataset
...
def build_trainer(param: HomoNLPParam, data, should_label_align=True, trainer=None):
...
class PytorchFederatedAggregator(object):
    ...
    def fit(self, loss_callback):
        LOGGER.info("PytorchFederatedAggregator is fitting...")
...
def build_aggregator(param: HomoNLPParam, init_iteration=0):
    LOGGER.info("PytorchFederatedAggregator is building aggregator")
