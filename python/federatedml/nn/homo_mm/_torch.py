...
from federatedml.nn.backend.pytorch.data import TableDataSet, VisionDataSet, MMDataSet
...
from federatedml.param import HomoMMParam
...
from federatedml.nn.backend.pytorch.MMBT.mmbt_utils import JsonlDataset, get_image_transforms, get_labels, load_examples, collate_fn, get_multiclass_labels, get_multiclass_criterion


time_logger = getLogger("time")
...
class PyTorchSAClientContext(_PyTorchSAContext):
    ...
    def send_model(self, tensors, weight):
        time_logger.info("transfer begin")
...
class FedLightModule(pl.LightningModule):
    def __init__(
        ...
        if args['multiclass']:
            ...
            labels = [0, 1]
            num_labels = len(labels)
        transformer_config = AutoConfig.from_pretrained(args['model_name'], num_labels=num_labels)
    ...
    def forward(self, x):
        # return self.model(x)
        input_ids, attention_mask, input_modal, modal_start_tokens, modal_end_tokens, labels = x
        return self.model(
            input_modal,
            input_ids=input_ids,
    ...
    def training_step(self, batch, batch_idx):
        ...
        input_ids, attention_mask, input_modal, modal_start_tokens, modal_end_tokens, labels = batch
        # LOGGER.info(f'y: {y}')
        input_ids = input_ids.to(args['device'])
        attention_mask = attention_mask.to(args['device'])
        input_modal = input_modal.to(args['device'])
        modal_start_tokens = modal_start_tokens.to(args['device'])
        modal_end_tokens = modal_end_tokens.to(args['device'])
        ...
        # input_modal = x[2]
        # LOGGER.info(f'input_modal shape: {input_modal.shape}')

        # attention_mask = x[1]
        # LOGGER.info(f'attention_mask shape: {attention_mask.shape}')

        # modal_start_tokens = x[3]
        # LOGGER.info(f'modal_start_tokens shape: {modal_start_tokens.shape}')

        # modal_end_tokens = x[4]
        # LOGGER.info(f'modal_end_tokens shape: {modal_end_tokens.shape}')
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {"val_loss": loss, "val_accuracy": torch.Tensor([accuracy])}
    ...
    def validation_epoch_end(self, outputs):
        ...
        time_logger.info("train end")

    def training_epoch_end(self, outputs) -> None:
        pass

    def on_train_epoch_start(self) -> None:
        time_logger.info("train begin")
    ...
    def configure_optimizers(self):
        ...
        # )
        self.context.configure_aggregation_params(optimizer=self.optimizer)
...
class PyTorchFederatedTrainer(object):
    ...
    def get_label_mapping(self):
        LOGGER.info(f"label_mapping:{self.label_mapping}")
    ...
    def predict(self, dataset, batch_size):
        ...
        dataloader = torch.utils.data.DataLoader(
            dataset=dataset, batch_size=batch_size, num_workers=1, sampler = train_sampler, collate_fn=collate_fn
        )
        results = []
        for x in dataloader:
            outputs = self.pl_model(x)
            logits = outputs.logits
            pred = torch.nn.functional.softmax(logits, dim=1).cpu().detach().numpy()
            results.append(pred)
        ...
        LOGGER.info(f'pred_tbl: {pred_tbl}')
...
def make_dataset(data, **kwargs):
    ...
    dataset = MMDataSet(data.path, **kwargs)
    LOGGER.info('len1: {}, len2: {}'.format(dataset.get_keys(), dataset.__len__()))
    return dataset
...
def build_trainer(param: HomoMMParam, data, should_label_align=True, trainer=None):
...
def build_aggregator(param: HomoMMParam, init_iteration=0):
